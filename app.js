...
// iziserb-webhook рассылает по APNs.
...
async function init() {
  ...
  if ('serviceWorker' in navigator) {
    ...
    // Перезагрузка страницы когда новый SW берёт управление (гарантирует свежие файлы)
    let reloading = false;
    ...
    if (reg) {
      // Когда новый SW заканчивает установку — сразу говорим ему пропустить ожидание
...
// Механики по образцу приложения Ame! на нашем контенте VOCAB_CATEGORIES.
// ============================================================
const FC_BATCH = 10; // пауза «продолжим/хватит» каждые 10 слов
let fcState = {
  catId: null, cat: null, dir: 'sr2ru', mode: 'vote',
    ...
    return `
      <button class="fc-cat-card" onclick="showFlashDeck('${cat.id}')">
        ...
        <div class="fc-cat-body">
          <div class="fc-cat-title">${cat.icon || '🃏'} ${cat.title}</div>
          <div class="fc-cat-sub">${total} слов</div>
        </div>
...
function showFlashDeck(catId) {
  ...
  fcState.catId = catId;
  fcState.cat = cat;
  document.getElementById('fc-deck-title').textContent = `${cat.icon || '🃏'} ${cat.title}`;
  document.getElementById('fc-wordlist').innerHTML = cat.words.map(w => `
    <div class="fc-word-row">
      <div class="fc-word-sr">${w.serbian}${w.transcription ? ` <span class="fc-word-tr">[${w.transcription}]</span>` : ''}</div>
...
function startFlashVote(dir) {
  ...
  document.getElementById('fc-vote-title').textContent = `${cat.icon || '🃏'} ${cat.title}`;
  renderFlashVote();
...
function renderFlashVote() {
  ...
  const front = fcState.dir === 'sr2ru' ? w.serbian : w.translation;
  const back = fcState.dir === 'sr2ru' ? w.translation : w.serbian;
  document.getElementById('fc-vote-tag').textContent = fcState.dir === 'sr2ru' ? 'СЛОВО' : 'ПЕРЕВОД';
  document.getElementById('fc-vote-front').textContent = front;
  document.getElementById('fc-vote-front-tr').textContent = (fcState.dir === 'sr2ru' && w.transcription) ? `[${w.transcription}]` : '';
  const backEl = document.getElementById('fc-vote-back');
  backEl.textContent = back;
  backEl.style.display = 'none';
  document.getElementById('fc-vote-hint').style.display = '';
...
function answerFlashVote(known) {
  ...
  if (known) {
    fcState.know++;
    if (!set.includes(w.serbian)) set.push(w.serbian);
  } else {
    fcState.dont++;
    const i = set.indexOf(w.serbian); if (i >= 0) set.splice(i, 1);
...
function finishFlashVote() {
  ...
  document.getElementById('fc-vote-hint').style.display = '';
  document.getElementById('fc-vote-hint').textContent = `Готово! Знаешь ${fcState.know} из ${total}`;
...
function renderFlashViewCard() {
  ...
  fcState.resume = false;
  fcState.currentWord = w;
  document.getElementById('fc-view-word').textContent = w.serbian;
...
// Озвучка слова (в приложении сербский голос может отсутствовать — тогда молча тихо).
function playFlashWord() {
  try {
    const w = fcState.currentWord;
    if (!w || !window.speechSynthesis) return;
    const u = new SpeechSynthesisUtterance(w.serbian);
    u.lang = 'sr-RS';
    const voices = speechSynthesis.getVoices() || [];
    const v = voices.find(x => /^(sr|hr|bs)/i.test(x.lang));
